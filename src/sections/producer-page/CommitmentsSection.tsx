...
import { Target, Shield, Rocket, Users, Star, TrendingUp } from 'lucide-react';
...
const commitments = [
  {
    ...
    description: "Chez Tiny Team, nous préparons chaque humoriste pour qu'il offre plus qu'un spectacle : une performance qui marque les esprits. Nos talents arrivent sur scène avec une énergie, une générosité et une maîtrise qui font la différence.",
    features: [
      "Artistes accompagnés et préparés",
      "Performances calibrées",
      "Expérience scénique confirmée",
      "Répertoire adapté à votre public"
    ],
  ...
  },
  {
    ...
    description: "Vous avez déjà assez à gérer. Avec Tiny Team, tout est cadré : interlocuteur unique, process simplifié, planning millimétré. La sérénité est notre standard.",
    features: [
      "Interlocuteur dédié",
      "Process optimisés",
      "Planning détaillé",
  ...
  },
  {
    ...
    description: "Chaque date est portée par notre équipe marketing pour maximiser l'impact. De la communication aux réseaux sociaux, Tiny Team amplifie la portée de vos événements pour remplir vos salles.",
    features: [
      "Stratégie marketing dédiée",
...
const CommitmentCard = ({ commitment, index }) => (
  ...
  >
    ...
    <motion.div
      ...
      style={{
        background: `radial-gradient(circle, ${commitment.glowColor} 0%, transparent 70%)`
      }}
      animate={{
        scale: [1, 1.2, 1],
        opacity: [0, 0.3, 0]
    ...
    <div className="relative glass-card rounded-3xl p-8 md:p-12 overflow-hidden h-full transform-gpu">
      ...
      <div className="absolute inset-0">
        <div className={`absolute inset-0 bg-gradient-to-br ${commitment.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
      ...
      <div className="relative space-y-6">
        ...
        <div className="mb-8">
          <div className="relative w-16 h-16 rounded-2xl glass-effect flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
            <commitment.icon className="w-8 h-8 text-pink-400 group-hover:text-pink-300 transition-colors duration-300" />
          </div>
        </div>
...
export const ProducerCommitmentsSection = () => {
  return (
